...
5. **Infrastructure and art never mix in one pass** (rules 11/12); commits are small, logically grouped, and
   ...
   decision).
6. **Hardware honesty** (rule 7). The OptiPlex has never run the engine; Phase 1 ships a measurement runbook,
   Phase 2 executes it on the physical box, and no stage claims are made before Phase 6 runs there for real.
...
| 2 | OptiPlex first-light (user executes Phase-1 runbook on the box) | Validation | Feeds the Phase-1 gate |
...
Parallelism: Phase 4 may run alongside Phase 3 (offline, disjoint files). Everything else is sequential.
Every phase ends with the engine building clean, all existing scenes passing their smoke tests, and a working
tree whose new commits are phase-scoped only.
